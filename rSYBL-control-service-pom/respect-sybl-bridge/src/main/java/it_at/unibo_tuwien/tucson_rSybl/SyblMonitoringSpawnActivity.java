...
public class SyblMonitoringSpawnActivity extends AbstractSpawnActivity  {
   ...
   /**
    ...
    public void doActivity() {
        ...
        LogicTuple mt;
        mt = this.in(new LogicTuple("monitoringTime",
			        new Value("T")));
        ...
        final MonitoringInterface monitor = OfferedMonitoredMetrics.getInstance(node);
        ...
        for (final String key : readMetrics.keySet()) {
            try {
                this.out(LogicTuple.parse("metric(node('" + node.getId()
                        + "'), name(" + key + "), value("
                        + readMetrics.get(key) + "))"));
            } catch (final InvalidLogicTupleException | TucsonInvalidLogicTupleException e) {
                // cannot happen
                e.printStackTrace();
            }
        }
    	}
    	catch (TucsonInvalidLogicTupleException | InvalidVarNameException e1) {
