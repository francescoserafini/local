...
import at.ac.tuwien.dsg.csdg.*;
import at.ac.tuwien.dsg.rSybl.dataProcessingUnit.utils.*;
...
/**
 * @author fra based on SyblScaleOutAgent
 * 
 */
public class SyblScaleInAgent extends AbstractTucsonAgent {
    public static final String AID = "'$rSYBL-scalein'";
     ...
     * @param n
     *            the SYBL Node to scalein
     * @throws TucsonInvalidAgentIdException
     *             if given id is not a valid TuCSoN agent id
     */
    public SyblScaleInAgent(final String id, final int p, final Node cs,final Node n) throws TucsonInvalidAgentIdException {
        super(id, SyblScaleInAgent.HOST, p);
        this.controlledService = cs;
        this.node = cs;
        try {
            this.tid = new TucsonTupleCentreId(SyblScaleInAgent.TC, super.myNode(), String.valueOf(super.myport()));
        } catch (final TucsonInvalidTupleCentreIdException e) {
           e.printStackTrace();
        }
        SyblScaleInAgent.bootLogging();
        this.bootClient();
        this.bootServer();
   ...
   private void bootClient() {
    ...
    /**
     * Override to change scaling in policy
     * 
     * @param entity
     *            the node to scale
     * @param controller
     *            the controller node
     * @return the ip of the new node or @code{"ERR"} if scale in failed
     * @throws InvalidVarNameException 
     */
    // copy-paste from JCloudsOpenStackConnection
    protected String scaleInAndWaitUntilNewServerBoots(final Node entity, final Node controller) throws InvalidVarNameException {
        // preparatory operations
        final CreateServerOptions createNodeOptions = SyblScaleInAgent
        ...
        try {
            // how many tries?
            ITucsonOperation op = this.acc.in(this.tid, new LogicTuple("scaleInTries", new Var("Tries")), Long.MAX_VALUE);
            tries = op.getLogicTupleResult().getArg(0).intValue();
            // which time step between re-tries?
            op = this.acc.in(this.tid, new LogicTuple("scaleInTime", new Var("Time")), Long.MAX_VALUE);
        ...
        if ("success".equals(res)) {
            ...
            RuntimeLogger.logger.error("Error when scaling in");
            ip = "ERR";
        }
    ...
    private void scaleInComponent(final Node n) throws InvalidVarNameException {
        final DependencyGraph graph = new DependencyGraph();
        graph.setCloudService(this.controlledService);
        final String ip = this.scaleInAndWaitUntilNewServerBoots(n,
    ...
    protected void main() {
        ...
        this.monitor.notifyControlActionStarted("ScaleIn", this.node);
        RuntimeLogger.logger.info("Scaling in ... " + this.node + " "
                + this.node.getNodeType());
        // only scale out target supported atm
        if (this.node.getNodeType() == NodeType.SERVICE_UNIT) {
            try {
				this.scaleInComponent(this.node);
			} catch (InvalidVarNameException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
        }
        this.monitor.notifyControlActionEnded("ScaleIn", this.node);
    }
}